...
class Validate_Models:
    ...
    def train_validate_cnn(self,N, trials, tam, s= 'val'):
        ...
        print("Validation accuracy N= [2,3,4,5,7,10]:", list_acc)
        return list_acc
    ...
    def train_validate_models(self, N, trials, tam, model, s= 'val'):
        ...
        for n in range(2,N+1):
            for t in range(trials):
                ...
                if (model == 'SVM'):
                    reg = svm.SVC(kernel = kernel, probability = True)
                if (model == 'RF'):
                    reg = RandomForestClassifier(max_depth=None, random_state=0, max_features = 100, n_estimators = 150)
                if (model == 'MLP'):
                    reg = MLPClassifier(solver='adam', alpha=1e-4,hidden_layer_sizes=(100,50,30), random_state=1)
        ...
        print("Validation accuracy N = [2,3,4,5,7,8,9,10]:", list_acc)
