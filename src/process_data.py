...
class Process_Data:
    ...
    def ClusterFingerprints(self,fps,cutoff=0.2):
        ...
        cluster_data = Butina.ClusterData(distances,lenfps,cutoff,isDistData=True)
        return cluster_data
    
    def plot_cluster(self, clusters):
    ...
    def process_data(self, csv_data, txt_file, data_size, smile_size, group_size):
        ...
        print("Number of valid SMILES:", len(drug_mol))
        
        drug_fps = [AllChem.GetMorganFingerprintAsBitVect(x,2,1024) for x in drug_mol]
        
        clusters=self.ClusterFingerprints(drug_fps,cutoff=0.7) #consider a cutoff value of 0.7 for Tanimoto similarity
        
        plot = self.plot_cluster(clusters)
